 ...
 * Fixed: Better handling of dates and missing fields, including updatedAt
 */
export function mapStrapiArticleToArticle(
  ...
  try {
    ...
    const description =
      strapiArticle.excerpt || extractTextFromHTML(combinedContent);
    ...
    return {
      ...
      updatedAt: strapiArticle.updatedAt || strapiArticle.createdAt, // Added missing updatedAt field
      ...
      seo: {
        metaTitle:
          ...
          strapiArticle.title ||
          "CMI Hospital Article",
        metaDescription:
          strapiArticle.seo?.metaDescription ||
          description ||
          "Read more on CMI Hospital",
        metaRobots: strapiArticle.seo?.metaRobots || "index,follow",
      },
      relatedArticles:
        strapiArticle.relatedArticles?.map(mapStrapiArticleToArticle) || [],
    };
  } catch (error) {
    ...
    return {
      ...
      updatedAt: new Date().toISOString(), // Added missing updatedAt field in fallback
 ...
 */
export function mapStrapiCategoryToCategory(
...
export function getImageUrl(article: Pick<Article, "image">): string {
  ...
  const baseUrl =
    process.env.NEXT_PUBLIC_STRAPI_URL || "http://cms-dev.cmihospital.com";
  return article.image.startsWith("/")
...
export function generateArticleStructuredData(article: StructuredDataArticle) {
  ...
  return {
    ...
    wordCount: cleanHtmlContent(article.content)
      .split(/\s+/)
      .filter((word) => word.length > 0).length,
