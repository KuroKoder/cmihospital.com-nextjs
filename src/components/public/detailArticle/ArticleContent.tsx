...
import { Share2, BookmarkPlus, Heart, Clock, List, AlertTriangle } from "lucide-react";
import { useTableOfContents, TableOfContentsItem } from "./TableOfContentGenerarator";
...
const ArticleContent: React.FC<ArticleContentProps> = ({ content, title }) => {
    ...
    tocResult.items.forEach((item, index) => {
      if (item.type === 'heading') {
        ...
        const headingRegex = new RegExp(
          `<h${item.level}([^>]*)>([^<]*${item.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').substring(0, 20)}[^<]*)</h${item.level}>`,
          'i'
        );
        processed = processed.replace(headingRegex, `<h${item.level} id="${item.id}"$1>$2</h${item.level}>`);
      } else {
        // For fallback/auto items, add invisible anchor points
        const searchText = item.title.substring(0, 15).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const anchorRegex = new RegExp(`(${searchText}[^<]{0,20})`, 'i');
        
        if (anchorRegex.test(processed)) {
          processed = processed.replace(anchorRegex, `<span id="${item.id}" class="toc-anchor"></span>$1`);
        } else {
          // If we can't find the text, add anchor at reasonable intervals
          const contentLength = processed.length;
          const position = Math.floor((index / tocResult.items.length) * contentLength);
          const insertPoint = processed.indexOf('>', position);
          if (insertPoint !== -1) {
            processed = processed.slice(0, insertPoint + 1) + 
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const sections = tocResult.items.map(item => {
                ...
                <div className="sticky top-32">
                  <div className="bg-gray-50 rounded-xl p-6 space-y-2">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className={`text-lg font-semibold ${methodInfo.color} flex items-center`}>
                    ...
                    {tocResult.method !== 'headings' && (
                      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        <div className="flex items-center text-blue-800">
                          ...
                          <span className="text-xs">{methodInfo.description}</span>
                        </div>
                      </div>
                    )}
                        ...
                        <button
                          ...
                          className={`block text-left text-sm transition-all w-full p-3 rounded-lg group ${
                            activeSection === item.id
                              ? `${methodInfo.color} bg-opacity-10 font-medium shadow-sm border-l-3 ${
                                  methodInfo.color.includes('green') ? 'border-green-500' :
                                  methodInfo.color.includes('blue') ? 'border-blue-500' :
                                  'border-purple-500'
                                }` 
                              : 'text-gray-600 hover:text-gray-800 hover:bg-gray-100'
                          }`}
                          style={{ paddingLeft: `${Math.max(12, (item.level - 1) * 8 + 12)}px` }}
                        >
              ...
              {tocResult.items.length > 0 && methodInfo && (
                <div className="lg:hidden bg-gray-50 rounded-xl p-4 mb-8">
                  <details className="group">
                    <summary className={`flex items-center justify-between cursor-pointer text-lg font-semibold ${methodInfo.color}`}>
                    ...
                    <nav className="mt-4 space-y-1">
                      {tocResult.items.map((item) => (
                        <button
                          ...
                          style={{ paddingLeft: `${Math.max(8, (item.level - 1) * 8 + 8)}px` }}
                        >
                          <span className="flex-1">{item.title}</span>
                          {item.type !== 'heading' && (
                            <span className="text-xs opacity-60 ml-2 flex-shrink-0">
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="text-center sm:text-left">
                      ...
                      <p className="text-xs text-gray-500">
                        Berikan feedback untuk membantu kami meningkatkan kualitas artikel
                      </p>
                    </div>
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-yellow-800 text-sm leading-relaxed">
                      Informasi dalam artikel ini bersifat edukatif dan tidak menggantikan konsultasi medis profesional. 
                      Selalu konsultasikan kondisi kesehatan Anda dengan dokter atau tenaga medis yang qualified 
                      sebelum mengambil keputusan terkait pengobatan atau perubahan gaya hidup.
                    </p>
                  </div>
                </div>
              </div>
